...
export default function PricingCardTwo({
  ...
  return (
    <section aria-label={`${name} plan`} className={cn('relative', className)}>
      {/* colored frame */}
      <div className={cn('rounded-3xl p-[2px]', t.frame)}>
        {/* card surface */}
        <div className='rounded-[22px] bg-white/95 dark:bg-zinc-950/90 backdrop-blur-2xl px-5 pb-5 pt-6 shadow-[0_8px_32px_rgba(31,38,135,0.05)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] h-full flex flex-col transition-all hover:scale-[1.02]'>
          {/* icon */}
          {icon && (
            <div
              className={cn(
                'mb-2 grid place-items-center text-3xl',
                t.accent,
                iconClassName
              )}
              aria-hidden
          ...
          <h3
            className={cn(
              'text-center text-lg font-semibold text-zinc-900 dark:text-zinc-50',
              nameClassName
            )}
          >
            {name}
          ...
          {subtitle && (
            <p
              className={cn(
                'mt-1 text-center text-xs text-zinc-500 dark:text-zinc-400',
                subtitleClassName
              )}
            >
              {subtitle}
          ...
          {/* price */}
          <div className={cn('mt-4 text-center', t.accent, priceClassName)}>
            <span className='text-3xl font-bold leading-none'>{pp.main}</span>
            <span
              className={cn(
                'ml-1 text-zinc-500 dark:text-zinc-400 text-xs',
                periodClassName
              )}
            >
              {periodLabel}
          ...
          {/* features */}
          <ul className={cn('mt-5 space-y-2.5 flex-1', featureListClassName)}>
            {features.map((f, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={cn(
                      'mt-0.5 inline-grid h-5 w-5 place-items-center rounded-full ring-1 shrink-0',
            ...
            <div className="mt-5">
              {cta.href ? (
                <a
                  ...
                  className={cn(
                    'inline-flex w-full items-center justify-center rounded-full px-4 py-2.5 text-xs font-semibold transition',
                    t.btn,
                    t.btnHover,
                    ctaClassName
                  )}
